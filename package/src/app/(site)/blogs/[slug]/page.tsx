...
export async function generateStaticParams() {
  return existingBlogs.map((b) => ({ slug: b.slug }));
}

export async function generateMetadata({ params } ) {
  const { slug } = await params;
  const blog = existingBlogs.find((b) => b.slug === slug);
  if (!blog) return {};
  return {
...
export default async function BlogPostPage({ params }) {
  ...
  const blogContent = blogDetailsContent.find((c) => c.id === blog?.id);
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        <article className="bg-white rounded-2xl shadow-lg overflow-hidden">
          ...
          <div className="p-8">
            ...
            <div className="prose max-w-none" dangerouslySetInnerHTML={{ __html: blogContent.content }} />
          </div>

          <div className="bg-gray-100 p-8 border-t border-gray-200">
            <h3 className="text-2xl font-bold text-gray-900 mb-6">Related Posts</h3>
            <div className="grid md:grid-cols-2 gap-6">
              {existingBlogs.filter((p) => p.id !== blog.id && p.category === blog.category).slice(0,2).map((rel) => (
                <Link key={rel.id} href={`/blogs/${rel.slug}`} className="group bg-white rounded-lg overflow-hidden hover:shadow-lg transition-all">
                  <div className="p-4">
                    <h4 className="font-semibold text-gray-900 group-hover:text-blue-600 transition-colors line-clamp-2">{rel.title}</h4>
                    <p className="text-sm text-gray-600 mt-2">{rel.readTime}</p>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <div className="mt-12 text-center">
          <Link href="/blogs" className="inline-block bg-blue-600 text-white px-8 py-3 rounded-lg hover:bg-blue-700 transition-colors font-semibold">Back to All Blogs</Link>
        </div>
      </div>
    </section>
  );
